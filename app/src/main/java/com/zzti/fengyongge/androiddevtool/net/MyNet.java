...
import com.zzti.fengyongge.androiddevtool.encrypt.Sha1;
...
public class MyNet {
    ...
    public static MyNet Inst(Context context) {
    ...
    public void meetingASssistant(int pageIndex,final ApiCallback callback) {
        ...
        String s = getpParameter(map,sign);
        String method ="meeting/assistant?"+s;
        final String apiUri = NET_DOMAIN +  method;
        _getAsyn(apiUri, callback);
    }
    ...
    public void update(final ApiCallback callback) {
        ...
        String sortString = sort(publicKey,map);
        String sign = toMD5(sortString);
        String s = getpParameter(map,sign);
